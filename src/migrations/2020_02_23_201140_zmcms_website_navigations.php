...
/**
 * TA TABELA ODPOWIADA ZA ŁĄCZENIE RÓŻYCH OBIEKTÓW DO NAWIGACJI
 */
$tblName=$tblNamePrefix.'website_navigations_linker';
Schema::create($tblName, function($table){$table->string('nav_token', 70);});
...
Schema::table($tblName, function($table){$table->string('run', 70)->nullable();});
Schema::table($tblName, function($table){$table->string('link', 70)->nullable();});
Schema::table($tblName, function($table){$table->timestamp('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));});//Imię
Schema::table($tblName, function($table){$table->timestamp('updated_at')->default(DB::raw('CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP'));});//Imię
Schema::table($tblName, function($table) use ($tblNamePrefix){$table->foreign('nav_token')->references('token')->on($tblNamePrefix.'website_navigations')->onUpdate('cascade')->onDelete('cascade');});
Schema::table($tblName, function($table){$table->primary(['nav_token', 'obj_token'], 'zmcmswnconnkey');});
