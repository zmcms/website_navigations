	...
	**/
	$("#positions_list .content").on('click',"a[id^='position_del_']", function(e){
		...
		if(confirm( 'Usunąć pozycję "'+$('#position_name_'+res).html()+'"?' )){
			$.ajax({
				type: 'GET',
				url:  "/"+backend_prefix+"/zmcms_website_navigation_position_delete/"+$('#position_code_'+res).val(),
				processData: false,
				contentType: false,
...
$('#ajax_dialog_box_content').on('click', "#zmcms_website_navigation_position_new_frm #btn_save", function(e){
	...
	$("#positions_list .content").on('click',"a[id^='position_edit_']", function(e){
		...
		e.preventDefault();e.stopPropagation();
			$.ajax({
				type: 'GET',
				url:  "/"+backend_prefix+"/zmcms_website_navigation_position_edit_frm/"+$('#position_code_'+res).val(),
				processData: false,
				contentType: false,
				success: function(data){
					$('#ajax_dialog_box').fadeIn( "slow", function() {});
	...
	**/	
	...
	$('#zw_nav_name_txt').on('keyup', function(e){
		$('#zw_nav_slug_txt').val(str_slug($('#zw_nav_name_txt').val()));
	});
	$('#zw_nav_link_txt').on('focusin', function(e){
		$.get(
			"/"+backend_prefix+"/zmcms_website_navigations_get_parent_link/"+$('#zcwn_hidden_parent').val(),
			function(data){
				$('#zw_nav_link_txt').val(data+'/'+str_slug($('#zw_nav_name_txt').val()));
			}
		);
		return false;
	});
	$('#ajax_dialog_box_content').on('click', 'a[id^="linker_btn_"]', function(e){
		e.preventDefault();e.stopPropagation();
		var o = $(this).attr('id'); 
		...
		$.get(
			"/"+backend_prefix+"/zmcms_website_navigations_linker_toggle/"+d.dataset.navtoken+"/"+d.dataset.navslug+'/'+d.dataset.linkedobjtoken+'/'+d.dataset.linkedobjtype+'/'+d.dataset.linkedobjslug,
			function(data){
				$('#ajax_dialog_box_content #'+o+' span:first-child').fadeOut( "slow", function() {});
				// $('#ajax_dialog_box_content #'+o+' span:first-child').prepend('<span class="far fa-dot-circle"></span>');
			}
		).done(function (data){
			$.get(
			"/"+backend_prefix+"/zmcms_website_navigations_linker_frm/"+d.dataset.linkedobjtoken+'/'+d.dataset.linkedobjtype+'/'+d.dataset.linkedobjslug,
				function (data){
					$('#ajax_dialog_box_content').html(data);
				}
			);
